...
class AudioSegment(object):
            ...
            out, _ = (ffmpeg.input(file,
                                   threads=0).output("-",
                                                     ...
                                                     ar=sample_rate).run(
                                                         ...
                                                         capture_stdout=True,
                                                         capture_stderr=True))
            samples = np.frombuffer(out, np.int16).flatten().astype(
                np.float32) / 32768.0
    ...
    def change_speed(self, speed_rate):
        ...
        self._samples = np.interp(new_indices, old_indices,
    ...
    def normalize(self, target_db=-20, max_gain_db=300.0):
        ...
        if gain > max_gain_db:
            raise ValueError("无法将段规范化到 %f dB，因为可能的增益已经超过max_gain_db (%f dB)" %
    ...
    def add_noise(self, noise, snr_dB, allow_downsampling=False, max_gain_db=300.0):
        ...
        if noise.sample_rate != self.sample_rate:
            raise ValueError("噪声采样率(%d Hz)不等于基信号采样率(%d Hz)" %
                             (noise.sample_rate, self.sample_rate))
        if noise.duration < self.duration:
            raise ValueError("噪声信号(%f秒)必须至少与基信号(%f秒)一样长" %
