...
class Train(object):
    ...
    def __init__(self, cfg):
        ...
        self.device = "cuda:{}".format(
            cfg.gpu_id) if torch.cuda.is_available() else "cpu"
    ...
    def epoch_train(self, epoch):
        ...
        for step, (inputs, labels) in enumerate(tqdm(self.train_loader)):
            ...
            if step % 50 == 0:
                ...
                print(
                    '[%s] Train epoch %d, batch: %d/%d, loss: %f, accuracy: %f，lr:%f'
                    % (datetime.now(), epoch, step, len(
                        self.train_loader), sum(loss_sum) / len(loss_sum),
                       sum(accuracies) / len(accuracies), lr))
        acc = sum(accuracies) / len(accuracies)
        loss = sum(loss_sum) / len(loss_sum)
        print("Train epoch:{:3.3f},Acc:{:3.3f},loss:{:3.3f}".format(
            epoch, acc, loss))
        print('=' * 70)
        return acc, loss
    ...
    def save_model(self, epoch, acc):
        ...
        model_path = os.path.join(self.model_dir,
                                  'model_{:0=3d}_{:.3f}.pth'.format(epoch, acc))
